...
@Component({
    ...
    templateUrl: './myCheckinTasks.component.html',
    providers: [ LocalStorage ]
    // styleUrls: ['./myTasks.component.less']
})

export class MyCheckinTasksComponent implements OnInit {
    userName: String;
    taskList: any = [];
    taskTotal = 0;
    ...
    onClickRefreshTask() {
        ...
        this.userName = currentUser.username;
        this._projectService.getMyCheckinTasks(this.userName).subscribe(data => {
    ...
    showSubmitFile(task) {
        localStorage.setItem('fileIDList', JSON.stringify(task.submitFileIDList));
        this._router.navigate(['/pages/watchList']);
    }
